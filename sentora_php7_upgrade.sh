...
echo ""
echo "############################################################################################"
echo "#  Welcome to the Unofficial Sentora PHP 7.2 upgrader. Installer v.$SENTORA_UPDATER_VERSION  #"
echo "############################################################################################"
echo ""
...
if [[ "$OS" = "CentOs" ]]; then
	...
	# START
	# -------------------------------------------------------------------------------
	echo -e "\nStarting PHP 7.2 with Packages update on Centos 6.*"	
	# -------------------------------------------------------------------------------
	...
	yum -y --enablerepo=remi-php72 install php php-zip
	...
	## Setup PHP 7.2 new PHP.INI file shipped with PHP and rename old PHP.INI
	...
	echo -e "\nFix missing php.ini settings sentora needs in CentOS 6.x php 7.2 ..."
	...
	# -------------------------------------------------------------------------------
	echo -e "\nStarting PHP 7.2 with Packages update on Centos 7.*"	
	# -------------------------------------------------------------------------------
	...
	# Fix missing php.ini settings sentora needs
	echo -e "\nFix missing php.ini settings sentora needs in CentOS 7.x php 7.2 ..."
	echo "setting upload_tmp_dir = /var/sentora/temp/"
	echo ""
	sed -i 's|;upload_tmp_dir =|upload_tmp_dir = /var/sentora/temp/|g' /etc/php.ini
	echo "Setting session.save_path = /var/sentora/sessions"
...
if [[ "$OS" = "Ubuntu" ]]; then
	...
	# START
	# -------------------------------------------------------------------------------
		echo -e "\nStarting PHP 7.2 with Packages update on Ubuntu 16.04"
	# -------------------------------------------------------------------------------	
		...
		#apt install php-pear php7.2-curl php7.2-dev php7.2-gd php7.2-mbstring php7.2-zip php7.2-mysql php7.2-xml php7.2-fpm libapache2-mod-php7.2 php7.2-imagick php7.2-recode php7.2-tidy php7.2-xmlrpc php7.2-intl
		
		apt-get -yqq install php7.2 php7.2-common 
		apt-get -yqq install php7.2-mysql php7.2-mbstring
		apt-get -yqq install php7.2-zip php7.2-xml php7.2-gd
		apt-get -yqq install php7.0-dev libapache2-mod-php7.2
		apt-get -yqq install php7.2-dev
		apt-get -yqq install php7.2-curl
		
		# PHP Mcrypt 1.0.2 install
		if [ ! -f /etc/php/7.2/apache2/conf.d/20-mcrypt.ini ]
				then
		...
		sed -i 's|;date.timezone =|'"$TIMEZONE"'|g' /etc/php/7.2/apache2/php.ini
		
		# Fix missing php.ini settings sentora needs
		echo -e "\nFix missing php.ini settings sentora needs in Ubuntu 16.04 php 7.2 ..."
		echo "setting upload_tmp_dir = /var/sentora/temp/"
		echo ""
		sed -i 's|;upload_tmp_dir =|upload_tmp_dir = /var/sentora/temp/|g' /etc/php/7.2/apache2/php.ini
		echo "Setting session.save_path = /var/sentora/sessions"
		sed -i 's|;session.save_path = "/var/lib/php/sessions"|session.save_path = "/var/sentora/sessions"|g' /etc/php/7.2/apache2/php.ini
		
		# Curl CERT Setup in PHP.ini files for PHP CURL_OPT
		echo -e "\nSetting up PHP.ini curl CERT..."
		wget https://curl.haxx.se/ca/cacert.pem
		mv cacert.pem /etc/php/7.2/cacert.pem
		sed -i 's|;curl.cainfo =|curl.cainfo = "/etc/php/7.2/cacert.pem"|g' /etc/php/7.2/apache2/php.ini
		sed -i 's|;openssl.cafile=|openssl.cafile = "/etc/php/7.2/cacert.pem"|g' /etc/php/7.2/apache2/php.ini
	...
	if  [[ "$PHPVER" = "7.2" ]]; then
    	echo -e "\nPHP 7.2 installed. Procced installing ..."
	else
		echo -e "\nPHP 7.2 not installed. Exiting installer. Please contact script admin"
		exit 1
	fi
	...
	if [[ "$OS" = "CentOs" && ( "$VER" = "6" || "$VER" = "7" ) ]]; then
		...
		ln -s /etc/php/7.2/mods-available/snuffleupagus.ini /etc/php/7.2/apache2/conf.d/20-snuffleupagus.ini
...
# -------------------------------------------------------------------------------
	...
	# Start
		if [[ "$(versioncheck "$phpver")" < "$(versioncheck "5.5.0")" ]]; then
			...
			while true; do
			read -e -p "PHPmyadmin 4.0.x will not work properly with PHP 7.2. Installer is about to upgrade PHPmyadmin to 4.9.1. Installer will backup current PHPmyadmin 4.x to /phpmyadmin_old incase something goes wrong. Do you want to keep the (O)riginal phpMyAdmin from Sentora or (U)pdate to the last stable version ? UPGRADE IS STRONGLY RECOMMENDED. (O/U)" pma
			echo ""
...
echo -e "\nDone updating all Sentora_core and PHP 7.2 files"
echo -e "\nEnjoy and have fun testing!"
echo -e "\nWe are done upgrading Sentora 1.0.3 - PHP 5.* w/Suhosin to PHP 7.2 w/Snuffleupagus"
